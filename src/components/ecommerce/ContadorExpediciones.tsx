...
import { Route } from "lucide-react";          // icono hoja de ruta / expedición
import { useCounter } from "@/hooks/useCounter";
import LoadingDots from "@/components/common/LoadingDots";

export default function ContadorExpediciones({ className = "" }: { className?: string }) {
    /* 1) React Query: total + estado de refetch */
    const {
        data: total,
        isLoading,
        isFetching,
        ...
        "https://testzone.cvx-r.cl/backend/backend.php?action=total_expediciones"
    );

    /* 2) Animar 0 → total cuando cambia */
      ...
      `}
        >
            <div className="flex items-center gap-2">
                <Route className="h-5 w-5 text-brand-500 shrink-0" />
